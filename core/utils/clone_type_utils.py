"""
Утилиты для управления типами клонирования.
"""

import bpy
from ...models.cloners import CLONER_TYPES, CLONER_NODE_GROUP_PREFIXES

def handle_clone_type_switch(obj, modifier, old_type, new_type):
    """
    Обрабатывает переключение между типами клонирования (объект/коллекция).
    
    Args:
        obj (bpy.types.Object): Объект с модификатором клонера
        modifier (bpy.types.Modifier): Модификатор клонера
        old_type (str): Предыдущий тип клонирования ('OBJECT' или 'COLLECTION')
        new_type (str): Новый тип клонирования ('OBJECT' или 'COLLECTION')
    
    Returns:
        bool: True если переключение успешно, False в случае ошибки
    """
...
def save_cloner_settings(modifier, clone_type):
    """
    Сохраняет текущие параметры клонера для указанного типа клонирования.
    
    Args:
        modifier (bpy.types.Modifier): Модификатор клонера
        clone_type (str): Тип клонирования ('OBJECT' или 'COLLECTION')
    """
    if not modifier or not modifier.node_group:
        return
...
def load_cloner_settings(modifier, clone_type):
    """
...
def restore_visibility_for_type_switch(obj, old_type, new_type):
    """
...
def update_node_interface_for_type(modifier, clone_type):
    """
